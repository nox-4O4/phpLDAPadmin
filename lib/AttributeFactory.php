					...
					default:
						if (! $_SESSION[APPCONFIG]->getValue('appearance','hide_template_warning'))
							system_message(array(
								'title'=>sprintf('%s [<i>%s</i>]',_('Unknown template [post] function'), htmlspecialchars($matches[1])),
								'body'=>sprintf('%s <small>[%s]</small>',_('The template function is not known and will be ignored.'), htmlspecialchars($values['post'])),
								'type'=>'warn'));
		...
		if (isset($values['helper']['value']) && ! is_array($values['helper']['value']))
			if (preg_match('/^=php\.(\w+)\((.*)\)$/',$values['helper']['value'],$matches))
				if (! in_array($matches[1],array('GetNextNumber','PasswordEncryptionTypes'))) {
					if (! $_SESSION[APPCONFIG]->getValue('appearance','hide_template_warning'))
						system_message(array(
							'title'=>sprintf('%s [<i>%s</i>]',_('Unknown template helper function'), htmlspecialchars($matches[1])),
							'body'=>sprintf('%s <small>[%s]</small>',_('The template helper function is not known and will be ignored.'), htmlspecialchars($values['helper']['value'])),
							'type'=>'warn'));
			...
			if (preg_match('/^=php\.(\w+)\((.*)\)$/',$values['value'],$matches)) {
				switch ($matches[1]) {
					...
					default:
						if (! $_SESSION[APPCONFIG]->getValue('appearance','hide_template_warning'))
							system_message(array(
								'title'=>sprintf('%s [<i>%s</i>]',_('Unknown template function'), htmlspecialchars($matches[1])),
								'body'=>sprintf('%s <small>[%s]</small>',_('The template function is not known and will be ignored.'), htmlspecialchars($values['value'])),
								'type'=>'warn'));
