...
if ($add_result) {
	...
	if ($action_number == 1 || $action_number == 2) {
		$create_message = sprintf('%s %s: <b>%s</b> %s',
			_('Creation successful!'),_('DN'), htmlspecialchars($request['template']->getDN()),_('has been created.'));
		...
		$request['page']->drawSubTitle(sprintf('%s: <b>%s</b>&nbsp;&nbsp;&nbsp;%s: <b>%s</b>',
			_('Server'), htmlspecialchars($app['server']->getName()),_('Distinguished Name'), htmlspecialchars($request['template']->getDN())));
